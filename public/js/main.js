...
socket.on('game_update',function(payload){
  ...
  /*check for good board paload*/
  var board = payload.game.board;
  if ('undefined' == typeof board || !board){
    console.los('Internal error: received a malformed board update from server');
    return;
  }
  /*update my color*/
  ...
  if (socket.id == payload.game.player_white.socket){
    ...
    /*somethine weird is going on, like three people playing at once*/
    /*send a client back to the lobby*/
    window.location.href ='lobby.html?username='+username;
    return;
  }
...
socket.on('play_token_response',function(payload){
  ...
  /*check for a play token response*/
  if (payload.result == 'fail'){
    console.log(payload.message);
    alert(payload.message);
    return;
